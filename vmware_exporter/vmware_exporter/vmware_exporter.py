...
class VmwareCollector():
    ...
    def __init__(
        ...
        self._labelNames = {
            'vms': ['vm_name', 'ds_name', 'host_name', 'dc_name', 'cluster_name','vc_name'],
            'vm_perf': ['vm_name', 'ds_name', 'host_name', 'dc_name', 'cluster_name','vc_name'],
            'vmguests': ['vm_name', 'ds_name', 'host_name', 'dc_name', 'cluster_name','vc_name'],
            'snapshots': ['vm_name', 'ds_name', 'host_name',  'dc_name', 'cluster_name','vc_name'],
            'datastores': ['ds_name', 'dc_name', 'ds_cluster','vc_name'],
            'hosts': ['host_name', 'dc_name', 'cluster_name','vc_name'],
            'host_perf': ['host_name', 'dc_name', 'cluster_name', 'vc_name'],
        }
    ...
    def _create_metric_containers(self):
        ...
        metric_list['vms'] = {
            ...
            'vmware_vm_up_timestamp_seconds': GaugeMetricFamily(
                'vmware_vm_up_timestamp_seconds',
        ...
        metric_list['vmguests'] = {
            ...
            'vmware_vm_perf_score': GaugeMetricFamily(
                ...
                labels=self._labelNames['vmguests']),
        }
        ...
        metric_list['hosts'] = {
            ...
            'vmware_host_maintenance_mode': GaugeMetricFamily(
                ...
                labels=self._labelNames['hosts']),
            'vmware_host_up_timestamp_seconds': GaugeMetricFamily(
                'vmware_host_up_timestamp_seconds',
    ...
    def datastore_labels(self):
        ...
        for dc in dcs:
            ...
            """
            append vcenter ip info 
            """
            for node in result:
                result[node] = result[node] + [os.environ.get('VSPHERE_HOST')]
    ...
    def vm_labels(self):
        ...
        for moid, row in virtual_machines.items():    
            ...
            if labels_cnt < len(self._labelNames['vms']):
                logging.info(
                    "Only ${cnt}/{expected} labels (vm, host, dc, cluster vcenter) found, filling n/a"
                    .format(
                        cnt=labels_cnt,
    ...
    def _vmware_full_snapshots_list(self, snapshots):
        ...
        for snapshot in snapshots:
            try:
                ...
                logging.error("get snapshot error :"+str(e))
                continue
    ...
    def _vmware_get_datastores(self, ds_metrics):
        logging.info('START: vmware get datastores metrics')
        ...
        for datastore_id, datastore in results.items():
            ...
            ds_metrics['vmware_datastore_usedpercent'].add_metric(
                labels, ds_usedper)
        ...
        logging.info('FINISH: vmware get datastores metrics')
        return results

    @defer.inlineCallbacks
    def _vmware_get_vm_perf_manager_metrics(self, vm_metrics):
        logging.info('START: vmware get vm perf manager metrics')
            ...
            for list_specs in chunks:
                ...
                for ent in results:
                    for metric in ent.value:
                        ...
                        if(metric_names[metric.id.counterId] == "vmware_vm_mem_usage_average"):
                            try:
                                updatenode = next(item for item in self.vm_score_list if item["vmname"] == labels[ent.entity._moId])
                                updatenode['memused'] = float(sum(metric.value))
                            except:
                                self.vm_score_list.append({"vmname": labels[ent.entity._moId], "memused": float(sum(metric.value))})
                        ...
                        vm_metrics[metric_names[metric.id.counterId]].add_metric(labels[ent.entity._moId], float(sum(metric.value)),)
        for item in self.vm_score_list:
            ...
            try:
                ...
                self.host_vm_overload.append({"hostname": hostname, "totalvm": 1, "cpureadyover":0 ,"diskover":0})
            if("cpuused" in item.keys() and item['cputotal']>0 and item['memtotal']>0):
                cpuused_score = 0
                memused_score = 0
                cpuready_score = 0
                disk_score = 0
                ...
                if(item['cpuready'] < 50):
                    ...
                    cpuready_score = 0
                    for updatenode in self.host_vm_overload:
                        if(updatenode["hostname"] == hostname):
                ...
                if("snapshot" in item.keys()):
                    ...
                    snapshot_score = 100
                vm_totalscore = cpuready_score * 0.15 + cpuused_score * 0.15 + memused_score * 0.3 + snapshot_score * 0.1 + disk_score*0.3
                vm_metrics["vmware_vm_perf_score"].add_metric(
                    item["vmname"],
                    float(vm_totalscore),
                )
        logging.info('FINISH:  vmware get vm perf manager metrics')

    @defer.inlineCallbacks
    def _vmware_get_host_perf_manager_metrics(self, host_metrics):
        global host_final_score
        logging.info('START: vmware get host perf manager metrics')
        ...
        # if redalert value >1 hostvalue = 0        
        ...
        for item in self.host_score_list:
            ...
            try:
                ...
                vmdisk_latencyper = specific_host['diskover'] / specific_host['totalvm']
                cpuusedper = item['cpuused']/item['cputotal']
                ...
                if vmcpu_readyper < 0.2 and vmcpu_readyper > 0.025:
                    warnnigvalue += 1
                elif vmcpu_readyper > 0.2:
                    criticalvalue += 1
                if criticalvalue >0:
                ...
                logging.error("calculate host score failed "+str(e))
        logging.info('FINISH:  vmware get host perf manager metrics')
    ...
    def _vmware_get_vms(self, metrics):
        ...
        Get VM information
        """
        logging.info("Start: vm metrics collection")
        ...
        for moid, row in virtual_machines.items():
            ...
            if 'runtime.powerState' in row:
                ...
                if power_state and row.get('summary.quickStats.uptimeSeconds'):
                    metrics['vmware_vm_up_timestamp_seconds'].add_metric(
                        labels,
                        row['summary.quickStats.uptimeSeconds']
                    )
        ...
        logging.info("FINISH: vm metrics collection")
    ...
    def _vmware_get_hosts(self, host_metrics):
        ...
        """
        logging.info("Start: host metrics collection")
        ...
        for host_id, host in results.items():
            ...
            if host.get('summary.quickStats.uptime'):
                # Host uptime
                host_metrics['vmware_host_up_timestamp_seconds'].add_metric(
                    labels,
                    host['summary.quickStats.uptime']
                )
            ...
            if 'summary.quickStats.overallMemoryUsage' in host:
                ...
                try:
                    ...
                    self.host_score_list.append(
                        {"hostname": labels, "memused": host['summary.quickStats.overallMemoryUsage']})
        ...
        logging.info("FINISH: host metrics collection")
        return results

    @defer.inlineCallbacks
    def _vmware_cluster_status(self,cluster_metics):
        logging.info("Start: cluster status")
        global host_final_score 
        global datastore_score 
        if self.fetch_tags:
            results, host_labels, host_tags = yield parallelize(
        ...
        logging.info("FINISH: cluster status")
...
class VMWareMetricsResource(Resource):
    ...
    def configure(self, args):
        ...
        for key in os.environ.keys():
            ...
            self.config[section.lower()] = {
                ...
                'fetch_alarms': get_bool_env('VSPHERE_{}_FETCH_ALARMS'.format(section), True),
                'collect_only': {
    ...
    def generate_latest_metrics(self, request):
        ...
        vchost = os.environ.get('VSPHERE_HOST')
        ssouser = os.environ.get('VSPHERE_USER')
        ssopwd = os.environ.get('VSPHERE_PASSWORD')
        try:
            si = connect.SmartConnectNoSSL(host = vchost, user=ssouser, pwd=ssopwd, port=443)
            atexit.register(Disconnect,si)
        except:
            request.setResponseCode(500)
            logging.info("login failed")
            request.write(b'login fialed please check username and password \n')
            request.finish()
            return
